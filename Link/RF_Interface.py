...
class RF_Interface:
    def __init__(self):
        ...
        self.tx_signal = None
        # cca threshold, -97dBm
        #self.cca_threshold = 1e-10
        self.cca_threshold = 1e-10
        self.cca_neighbours = []
        self.rx_paras = []
        self.rx_log = []
        RF_Interfaces.add(self)
    ...
    def rx_start(self, tx_interface, tx_psd, rx_duration, num_blocks):
        self.state = self.State.RX
        self.rx_paras = []
        for i in range(num_blocks):
            slot = rx_duration.sim_time/num_blocks
            Simulator.schedule(i*slot, self.rx_para_calc, tx_interface, tx_psd)

    def rx_para_calc(self, tx_interface, tx_psd):
        signals = RF_Signal.signals
        channel = self.channel
        interfer_psd = np.zeros(len(tx_psd))
        for signal in signals:
            if signal.interface != tx_interface:
                interfer_rx_psd = channel.est_trans_calc(
                    signal.interface, self, signal.tx_psd)
                interfer_psd += interfer_rx_psd
        rx_psd, _ = channel.transmission_calc(
            tx_interface, self, tx_psd)
    ...
    def rx_end(self, phy_frame):
        self.state = self.State.IDLE
        rx_sinrs = np.array([para.rx_sinr for para in self.rx_paras])
        #print('sinr',rx_sinrs[0])
        ...
        if random.random() <= prob_rx_success:
            print('node', self.node.id, 'wireless interface received at ',
            ...
            print('node', self.node.id, 'wireless interface decode failed at ',
                  Simulator.now().value, Simulator.now().type)
