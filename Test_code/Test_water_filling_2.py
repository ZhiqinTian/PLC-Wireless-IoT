...
P_w_all = 100
P_p_all = 100
snr_w_ref = np.round(np.clip(np.random.normal(10, 5, size=N), 1, 20), 2)
snr_p_ref = np.round(np.clip(np.random.normal(10, 5, size=N), 1, 20), 2)
...
def best_power_allocation(x):
    ...
    yw = P_w_d*yw_part/(snr_div+0.001*yw_part)+delta_w/Nr
    yp_part = (1+snr_div)*(1+snr_p_d * fw*Fp/(snr_w_d*fp*Fw))
    yp = P_p_d*yp_part/(snr_div+0.001*yp_part)+delta_p/Nr
...
#x_w = P_w[d]
#x_p = P_p[d]
R_W0 = np.log2(1+x_w*factor_w[d])
R_P0 = np.log2(1+x_p*factor_p[d])
x0 = np.concatenate(
    (x_w, x_p, np.power(2, R_W0+R_P0)-1), axis=0)
...
for i in range(len(d)):
    _,_,R_sc,_ = R_max(p_error,cnr_w_d[i]*P_w_d_new[i] , m,cnr_p_d[i]*P_p_d_new[i] , s)
    R_d_new.append(R_sc)
